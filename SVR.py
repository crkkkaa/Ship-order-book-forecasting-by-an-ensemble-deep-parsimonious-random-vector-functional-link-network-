...
def format_data(x_,y_,order):
    ...
    for i in range(n_sample):
        ...
        y[i,:]  =y_[i+order,:]
    return x,y
def SVR_pre(trainx,trainy,testx,hyper):
    order=hyper[0]
    gamma=hyper[1]
    C=hyper[2]
    epsilon=hyper[3]
...
def cv(x_,y_,pre_l):
    ...
    for order in orders:
        ...
        for i, h in enumerate(hypers):
            p=SVR_pre(trainx,trainy,testx,h)
...
for feats in [aframax,suezmax,panamax,capesize][:]:    
    ...
    for seed in np.arange(10):
        np.random.seed(seed)        
        model = SVR_pre(trainx, trainy, testx, bh)
        svrpre=SVR_pre(trainx,trainy,testx,bh)
        svrpre=yscaler.inverse_transform(svrpre.reshape(-1,1))
        predictions.append(svrpre)
    svrpre=np.concatenate(predictions,axis=1)
    svrpre=pd.DataFrame(svrpre)
    svrpre.to_csv(targf+'Step'+str(step)+'SVR.csv')
