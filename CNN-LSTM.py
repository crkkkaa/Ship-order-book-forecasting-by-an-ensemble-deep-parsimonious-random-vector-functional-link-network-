# -*- coding: utf-8 -*-
"""
Created on Tue Jul 25 14:16:14 2023
    ...
    for features in [panamax,aframax,suezmax,capesize][:]:
        ...
        for rnh in [2,4,8,16]:
            for ch in [1,2,3,4]:
                ...
                rcnn_hypers=list(product(chs,pools,orders,bs,weight_decays,lrs,epochss,rnn_hs))
                
                
        rcnn_best_h,rcv_loss=rcv(rcnn_hypers,x_,target_idx,validation_l)
        for seed in np.arange(10): 
            ...
            norm_pre,nnmodel=rcnn_pre(norm_dif, rcnn_best_h, test_l, False,False)
            df_pre=target_scaler.inverse_transform(norm_pre.reshape(-1,1))
            prediction=(df_pre+target[-1-test_l:-1].reshape(-1,1)).ravel()            
            rcnnpre.append(prediction)
        CNNLSTMpre = np.concatenate([np.reshape(arr, (-1, 1)) for arr in rcnnpre], axis=1)
        CNNLSTMpre=pd.DataFrame(CNNLSTMpre)
        CNNLSTMpre.to_csv(target_f+'Step'+str(step)+'CNNLSTM.csv')